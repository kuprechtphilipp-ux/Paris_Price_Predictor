...
def airbnb_page():
    ...
    # --- GeoJSON Loading: Using absolute path derived from execution context ---
    geojson_data = None
    GEOJSON_FEATURE_ID_KEY = "properties.c_arinsee"

    try:
        # 1. Wir ermitteln das Verzeichnis, in dem airbnb_page.py liegt (code/pages)
        current_script_dir = Path(__file__).parent
        
        # 2. Wir konstruieren den Pfad: Zwei Ordner hoch (code/) + data/ + dateiname
        geojson_path_absolute = current_script_dir.parent / "data" / "paris.geojson"
        
        # 3. Öffnen des GeoJSON mit dem absoluten Pfad
        with open(geojson_path_absolute, "r", encoding='utf-8') as f:
            geojson_data = json.load(f)
            
    except FileNotFoundError:
        st.error(f"Critical Error: GeoJSON file not found. Checked path: {geojson_path_absolute}. Map cannot be rendered.")
